...
namespace Presentation.Chapter
{
    public class Chapter2Handler : MonoBehaviour
    {
        ...
        private void Start()
        {
            _config = StoryScenarioLibrary.BuildChapter2();
        ...
        {
            _state = 5;
            if (string.IsNullOrEmpty(_config.TransitionDialogue)) { CompleteChapter(); return; }
        ...
        private void CompleteChapter()
        {
            ...
            if (data != null)
            {
                StoryProgressService.MarkChapterCompleted(data, ChapterIds.WanbeiManufacturing, 100);
                StoryProgressService.UnlockKnowledgeCards(data, _config.UnlockedCards);
                GameManager.Instance.AutoSaveActiveAccount();
            }
            EventBus.Trigger(new IntentEvent(IntentEvent.IntentType.GoToChapter, 3));
        }
        ...
        private void SaveCurrentProgress(int step, bool completed = false)
        {
            var data = GameManager.Instance.CurrentAccountData;
            if (data == null) return;
            var progress = data.ChapterProgresses.Find(p => p.ChapterId == ChapterIds.WanbeiManufacturing);
            if (progress == null) { progress = new ChapterProgress { ChapterId = ChapterIds.WanbeiManufacturing }; data.ChapterProgresses.Add(progress); }
